#!/usr/bin/env python3
"""
ZeroSync v6.0 – Ultimate Bluetooth Attack Toolkit
Author: Null_Lyfe
"""
...
def crash_device():
    ...
    idx = user_input = Prompt.ask("💥 Crash index", default="0")
...
def ble_deauth():
    devices = scan_devices()
    if not devices: return
    idx = user_input = Prompt.ask("✂️ Index to deauth", default="0")
    if not user_input.isdigit():
        print("⚠️ Invalid input. Defaulting to 0.")
        idx = 0
    else:
...
def cve_2017_0785():
    devices = scan_devices()
    if not devices: return
    idx = user_input = Prompt.ask("💣 CVE target index", default="0")
    if not user_input.isdigit():
        print("⚠️ Invalid input. Defaulting to 0.")
        idx = 0
    else:
...
def replay_notification():
    devices = scan_devices()
    if not devices: return
    idx = user_input = Prompt.ask("💌 Replay index", default="0")
    if not user_input.isdigit():
        print("⚠️ Invalid input. Defaulting to 0.")
        idx = 0
    else:
...
def rfcomm_flood():
    devices = scan_devices()
    if not devices: return
    idx = user_input = Prompt.ask("📡 Index for RFCOMM flood", default="0")
    if not user_input.isdigit():
        print("⚠️ Invalid input. Defaulting to 0.")
        idx = 0
    else:
...
def l2ping_dos():
    devices = scan_devices()
    if not devices: return
    idx = user_input = Prompt.ask("📶 L2Ping DoS index", default="0")
    if not user_input.isdigit():
        print("⚠️ Invalid input. Defaulting to 0.")
        idx = 0
    else:
...
def main_menu():
    while True:
        console.print(Panel("[bold cyan]ZeroSync v6.0 – Created by Null_Lyfe[/bold cyan]", border_style="bright_magenta"))
        ...
        elif choice == "0": console.print("[bold red]Exiting...[/bold red]"); break
