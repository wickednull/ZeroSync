...
#!/usr/bin/env python3
# ZeroSync v3.7 - Real Bluetooth Hacking Toolkit (No placeholders)
...
def launch_btlejack_jammer():
    ...
    try:
        ...
        console.print(f"[red]btlejack error:[/red] {e.output}")
    except FileNotFoundError:
        console.print("[red]btlejack not found. Install it first.[/red]")

def launch_gattacker_mitm():
    console.print(Panel("Starting GATTACKER (MITM Injection)", style="bold magenta"))
    try:
        os.system("xterm -e 'gattacker' &")
        console.print("[green]gattacker launched in a separate terminal.[/green]")
    except Exception as e:
        console.print(f"[red]Failed to launch gattacker:[/red] {e}")
def main_menu():
    while True:
        console.print(Panel("ZeroSync v3.6 - Real Bluetooth Toolkit", style="bold blue"))
        ...
        menu = [
            ...
            ("8", "Auto Attack Chain"),
            ("9", "Bettercap BLE Recon"),
            ("10", "BLE Jammer (btlejack)"),
            ("11", "GATT Injection MITM (gattacker)"),
            ("0", "Exit")
        ]

        for opt, tool in menu:
